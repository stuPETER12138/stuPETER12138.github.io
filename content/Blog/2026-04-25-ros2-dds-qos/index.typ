...
我参加的比赛要求在树莓派上使用视觉算法。为了更好的调试代码，一个可以实时显示图像的可视化界面必不可少。但是我并不想用虚拟机，恰好我的 Windows 电脑支持 WSLg，又恰好比赛用到的 ROS2 支持分布式通信（理论上我可以在 WSL2 上查看同一局域网内的树莓派上的 ROS2 话题等等），于是用 WSL2 查看树莓派的视觉图像的想法油然而生。然后我就发现用 WSL2 查看图像卡的不行，即使是 `/image/compressed` 下的压缩图像也有好几秒钟的延迟。询问 AI 后得知我应该换用 Cyclone DDS，那么，为什么呢？
...
它在航空航天和国防领域早已成熟，而如今已不再局限于这些领域。现代机器人系统既需要高吞吐量、低延迟控制系统，又要避免因中间引入消息代理而产生单点故障。DDS 已然是 ROS2 中使用最广泛、默认的中间件选择，用于在组件之间传输指令、传感器数据，甚至视频和点云数据。不同厂商实现了不同的 DDS 具体实现，ROS2 通过一个叫 RMW（ROS Middleware）的抽象层来灵活切换它们。

而我用到的 ROS2 Humble 默认使用 Fast DDS。它与 Cyclone DDS 有什么区别呢？D 老师#footnote[DeepSeek]帮我总结了以下三点：
...
- *WSL2 兼容性*：Fast DDS 在 WSL2 中经常出现节点发现失败、延迟剧烈抖动和 CPU 占用过高等问题。Cyclone DDS 则提供了简洁的 XML 配置文件，可以显式指定 WSL2 的虚拟网卡，直接绕过了大部分网络兼容性陷阱。
...
简单来说，最适合实时图像流的组合就是：*Best Effort + Keep Last（深度 1）+ Volatile*。当你在订阅端代码中将 QoS 配置成这样，配合 Cyclone DDS，即使跨 WSL2 查看树莓派上的图像，也能获得近乎实时的流畅体验。
